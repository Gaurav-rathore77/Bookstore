...
const ProductList = () => {
    ...
    useEffect(() => {
        setSortedProducts([...products]); // Update sortedProducts when products change
    }, [products]);

    const handleSortByPrice = () => {
        setSortedProducts((prevSortedProducts) =>
            [...prevSortedProducts].sort((a, b) => a.price - b.price)
        );
    };
    ...
    const handleFilterByType = () => {
        if (selectedType === "all") {
            ...
            const filtered = products.filter(
                (product) => product.genre?.toLowerCase() === selectedType.toLowerCase()
            );
            setSortedProducts(filtered);
        }
    };
